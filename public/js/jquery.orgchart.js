...
(function (factory) {
  ...
  OrgChart.prototype = {
      ...
      $chart.on('mousemove touchmove',function(e) {
        ...
        if (lastTf === 'none') {
          if (lastTf.indexOf('3d') === -1) {
			...
			/*let largura = (parseInt($('#chart-container .orgchart').width()) / 2) * (-1);
			if(newX >= -100){newX = -100;}
			if(newX <= largura){newX = largura;}
			
			let altura = (parseInt($('#chart-container .orgchart').height()) / 2) * (-1);
			if(matrix[3] == '0'){
				if(newY >= -100){newY = -100;}
				if(newY <= altura){newY = altura;}
			} else {
				if(newY >= (altura * (-1))){newY = altura * (-1);}
			}*/
    ...
    zoomWheelHandler: function (e) {		
		let modo
		try {
			modo = e.data.originalEvent.deltaY
		} catch(m){
			modo = e.originalEvent.deltaY
		}
		
		var oc = e.data.oc;
		e.preventDefault();
		var newScale  = 1 + (modo > 0 ? -0.05 : 0.05);
		oc.setChartScale(oc.$chart, newScale);
    },
    centralizarTela: function (e) {		
		/*CENTRALIZAR NA TELA*/ // 5 	= ?
		  // HOZIRONTAL			// 100  = 5500
		  let w1 = $('#chart-container .orgchart').width()
		  let w2 = $('.card-body').width()
		  let horizontal = -150;
		  
		  if(w1 <= w2){
			  horizontal = 0;
		  } else  if( ((w2 - 50) - w1) < 0) {
			horizontal = ((w2 - 50) - (w1 + 600)) / 2
		  }
		  
		  let matrixAtual =  $('#chart-container .orgchart').css('transform');
		  let newMatrix
		  let matrixBase	  
		  
		  let eventoOriginal
		  try {
			  eventoOriginal = e.data.originalEvent
		  } catch(e) {
			eventoOriginal = ''
		  }
		  
		  if(matrixAtual != 'none' && eventoOriginal != 'reset'){
			  matrixBase = matrixAtual.split(", ");
			  matrixBase[4] = horizontal;
			  
			  //VERTICAL
			  let zoom = parseFloat(matrixBase[3]).toFixed(2)
			  let vertical = parseInt((((zoom - 1) * 580000) / 100) - 300)
			  matrixBase[5] = vertical + ")"; 
			  
			  newMatrix = matrixBase.join(", ")
		  } else {
			  newMatrix = 'matrix(1, 0, 0, 1, ' + horizontal + ', -300)'
		  }
		  
		  try {
			  if(e.data.originalEvent == 'reset' || e.data.originalEvent == 'center'){
				$('html, body').scrollTop(0);
			  }
		  } catch(e){
		  }
		  
		  $('#chart-container .orgchart').css('transform', newMatrix);
		  /* FIM CENTRALIZAR TELA */
    },
    //
    ...
    bindZoom: function () {
		...
		$("#OcBtnZoomIn").on('click', { 'oc': this }, this.zoomWheelHandler);
		$("#OcBtnZoomOut").on('click', { 'originalEvent': {'deltaY' : 100}, 'oc': this }, this.zoomWheelHandler);
		$("#OcBtnCenter").on('click', { 'originalEvent': 'center', 'oc': this }, this.centralizarTela);
		$("#OcBtnReset").on('click', { 'originalEvent': 'reset', 'oc': this }, this.centralizarTela);
